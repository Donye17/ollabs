 ...
 * Sized for ~2x of common circle thumbs (88–128 CSS px). Failures are silent;
 * the save still counts without a photo.
 */
export async function uploadExploreThumb(canvas: HTMLCanvasElement): Promise<string | null> {
    ...
    const blob = await new Promise<Blob | null>((resolve) => off.toBlob(resolve, 'image/jpeg', 0.78));
    if (!blob) return null;

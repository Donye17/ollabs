...
- [x] 24 designed frames generated and installed
- [ ] Commit and deploy everything in the working tree
...
- [ ] Run `drizzle/seed_q3_2026_campaigns.sql` against production
...
- [ ] Promote Suicide Prevention, Childhood Cancer, Hispanic Heritage, and Alzheimer's frames (all seeded and designed)
...
- [ ] Three designed pink frames are live and indexed (ribbon, badge, subtle)
...
- [ ] Orange Unity Day frames pushed to schools. Schools plan late, so this timing is right
- [ ] Purple Domestic Violence frames pushed to shelters and advocacy orgs
...
## The frames shipped for this quarter

24 designed frames in `public/frames`, generated by `scripts/generate_frames.py`, seeded by `drizzle/seed_q3_2026_campaigns.sql`.

**Four archetypes, each built to survive being displayed at 32 to 48 pixels:**

- **Emblem** — the ring breaks and a properly constructed awareness ribbon is set into the gap. Used where the ribbon itself is the recognised symbol (breast cancer, childhood cancer, suicide prevention, Alzheimer's, domestic violence).
- **Crest** — the ring breaks and a solid capsule of tight bold type takes over. Type auto-sizes against the circular chord so it never gets clipped.
- **Sweep** — hairline ring with a tapered band across the bottom carrying the message. The band is sized to the text, not the other way round.
- **Hairline** — one clean ring and a single solid pip. For people who want to show support without a loud graphic. This is the frame people actually leave on for a month.

Four things separate these from a coloured circle, and they are documented in `scripts/frame_lib.py`:

1. **Taper.** Bands thin toward their ends. Blunt 90-degree cuts read as clipping errors.
2. **Angular gradient.** Colour travels along the arc's own path. A linear gradient on a curve always looks arbitrary because the axis has nothing to do with the shape.
3. **Shadow separation.** A soft shadow under the frame so it sits above the photo rather than in the same plane as the face.
4. **Nesting, not stacking.** Breaking the ring and setting the mark into the gap. Nesting reads as one designed object; a disc stacked on an unbroken ring reads as a sticker.

Ring weight is ~5 to 6% of diameter, two colours maximum, everything supersampled 4x and downsampled with LANCZOS, nothing crosses the circular safe zone. Output is quantized to 128 colours: 3.2MB down to 0.48MB with no visible loss. Verified legible at 128px, 64px, and 40px.

**Coverage:** Breast Cancer (3), Domestic Violence (2), Unity Day (2), World Mental Health Day (2), Suicide Prevention (2), Childhood Cancer (2), Hispanic Heritage, Alzheimer's, Homecoming, Class of 2027, Game Day, National Nonprofit Day, Giving Tuesday (2), Volunteer, Blood Donor, Supporter.

Add more by appending to the `build()` function and the `CAMPAIGNS` list, then re-running both scripts.
